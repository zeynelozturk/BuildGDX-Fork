...
public class ALEchoEffect {
    ...
    public ALEchoEffect() {
        ...
        if (alGetError() == AL_NO_ERROR) {
            EXTEfx.alEffecti(alEffect, EXTEfx.AL_EFFECT_TYPE, AL_EFFECT_ECHO);
        }
    }
    ...
    public void setReverb(boolean enabled, float delay) {
        ...
        if (enabled) {
            this.delay = BClipRange(delay, 0.0f, 1.0f);
        } else {
            this.delay = 0;
        }
    }
    ...
    public void setToSource(int soundId) {
        ...
        if (enabled) {
            EXTEfx.alEffecti(alEffect, EXTEfx.AL_EFFECT_TYPE, AL_EFFECT_ECHO);
            EXTEfx.alEffectf(alEffect, EXTEfx.AL_ECHO_DELAY, 0.0f); /* 0.0f ... 0.207f */
            EXTEfx.alEffectf(alEffect, EXTEfx.AL_ECHO_LRDELAY, 0.07f); /* 0.0f ... 0.404f */
            EXTEfx.alEffectf(alEffect, EXTEfx.AL_ECHO_DAMPING, 0.10f); /* 0.0f ... 0.99f */
            EXTEfx.alEffectf(alEffect, EXTEfx.AL_ECHO_FEEDBACK, delay); /* 0.0f ... 1.0f */
            EXTEfx.alEffectf(alEffect, EXTEfx.AL_ECHO_SPREAD, 0.0f); /* -1.0f ... 1.0f */
