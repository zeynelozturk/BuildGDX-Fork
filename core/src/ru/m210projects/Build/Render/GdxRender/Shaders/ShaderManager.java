...
public class ShaderManager {
    ...
    // Without it Shader.values() will create Shader array every frame in reset()
    private Shader[] shaders = Shader.values();
    ...
    public void init(TextureManager textureCache, int palette_numshades) {
        this.palette_numshades = palette_numshades;
    ...
    public boolean isInited() {
        return skyshader != null && skyshader32 != null && texshader != null && texshader32 != null && bitmapShader != null && fadeshader != null;
    }
    ...
    public void viewport(int cx1, int cy1, int cx2, int cy2) {
        ...
        if (shader == Shader.IndexedWorldShader) {
            ...
            texshader.setUniformf(world_viewport, cx1, Gdx.graphics.getBackBufferHeight() - cy1, cx2 + 1, Gdx.graphics.getBackBufferHeight() - cy2 - 1);
            ...
            texshader32.setUniformf("u_viewport", cx1, Gdx.graphics.getBackBufferHeight() - cy1, cx2 + 1, Gdx.graphics.getBackBufferHeight() - cy2 - 1);
        }
    }
    ...
    public void frustum(Plane[] clipPlane) {
        ...
        if (shader == Shader.IndexedWorldShader) {
            ...
            texshader.setUniformf(world_plane0, clipPlane[0].normal.x, clipPlane[0].normal.y, clipPlane[0].normal.z, clipPlane[0].d);

            // XXX world_plane1 doesn't find
            texshader.setUniformf("u_plane[1]", clipPlane[1].normal.x, clipPlane[1].normal.y, clipPlane[1].normal.z, clipPlane[1].d);
        } else if (shader == Shader.RGBWorldShader) { //XXX
            if (clipPlane == null) {
                texshader32.setUniformi("u_planeClipping", 0);
                return;
            ...
            texshader32.setUniformf("u_plane[0]", clipPlane[0].normal.x, clipPlane[0].normal.y, clipPlane[0].normal.z, clipPlane[0].d);
            texshader32.setUniformf("u_plane[1]", clipPlane[1].normal.x, clipPlane[1].normal.y, clipPlane[1].normal.z, clipPlane[1].d);
        }
    }
    ...
    public void prepare(BuildCamera cam, int windowx1, int windowy1, int windowx2, int windowy2) {
        Shader shader = this.getShader();
    ...
    public void prepare(Shader shader, BuildCamera cam, int windowx1, int windowy1, int windowx2, int windowy2) {
        check(shader);
    ...
    public IndexedSkyShaderProgram allocIndexedSkyShader(final TextureManager textureCache) {
        try {
            IndexedSkyShaderProgram skyshader = new IndexedSkyShaderProgram(palette_numshades) {
                @Override
                public void bind() {
                    super.bind();
                    currentShaderProgram = this;
            ...
            if (!skyshader.isCompiled()) {
                throw new IllegalArgumentException("Shader compile error: " + skyshader.getLog());
            }
