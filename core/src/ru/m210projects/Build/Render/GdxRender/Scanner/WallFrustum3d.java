...
public class WallFrustum3d implements Poolable {
    ...
    public BuildCamera getCamera() {
        return camera;
    }

    public WallFrustum3d set(BuildCamera cam, int sectnum, int width, int height) {
    ...
    private WallFrustum3d buildFrustum() {
        camera.unproject(tmpVec[0].set(bounds[0].x, bounds[0].y, 1));
        camera.unproject(tmpVec[1].set(bounds[0].x, bounds[1].y, 1));
        camera.unproject(tmpVec[2].set(bounds[1].x, bounds[1].y, 1));
        camera.unproject(tmpVec[3].set(bounds[1].x, bounds[0].y, 1));
    ...
    private void build(Plane p, Vector3 p1, Vector3 p2) {
        ...
        float posy = camera.getY();
        float posz = camera.getZ();

        p.normal.set(posx, posy, posz).sub(p1).crs(p1.x - p2.x, p1.y - p2.y, p1.z - p2.z);
//		p.normal.nor();
        p.d = -p.normal.dot(posx, posy, posz);
    }
    ...
    private Vector3[] calcBounds(ArrayList<? extends Vector3> points) {
        ...
        bounds[1].x = Math.min(bounds[1].x, width);
        bounds[0].y = Math.max(bounds[0].y, 0);
        bounds[1].y = Math.min(bounds[1].y, height);
    ...
    public Plane[] getPlanes() {
        ...
        camera.unproject(tmpVec[0].set(b0x, b0y, 1));
        camera.unproject(tmpVec[1].set(b0x, b1y, 1));
        camera.unproject(tmpVec[2].set(b1x, b1y, 1));
        camera.unproject(tmpVec[3].set(b1x, b0y, 1));
