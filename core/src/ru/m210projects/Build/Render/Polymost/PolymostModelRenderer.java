...
public class PolymostModelRenderer {
    ...
    public int voxdraw(GLVoxel m, Sprite tspr) {
        ...
        if (yflip && (globalorientation & 16) == 0) {
            k0 += ((parent.getTile(tspr.getPicnum()).getHeight() * 0.5f) - m.zpiv) * tspr.getYrepeat() * 8.0f;
        }

        f = (65536.0f * 512.0f) / (parent.xdimen * parent.viewingrange);
        g = 32.0f / (float) (parent.xdimen * parent.gxyaspect);
        ...
        modela0.y = ((x0 - parent.globalposx) / 1024.0f + modela0.y) * f;
        dvoxm0.x *= -f;
        if ((boardService.getSprite(tspr.getOwner()).getCstat() & 48) == 32) {
            dvoxm0.x *= -1;
        }
        ...
        modela0.x = ((tspr.getY() - parent.globalposy) / -1024.0f + modela0.x) * -f;
        dvoxm0.z *= g;
        if (yflip) {
            dvoxm0.z *= -1;
        }
        modela0.z = ((k0 - parent.globalposz) / -16384.0f + modela0.z) * g;
        ...
        polyColor.r = polyColor.g = polyColor.b = (numshades
                - min(max((parent.globalshade * parent.shadescale)/* + m.shadeoff */, 0), numshades)) / (numshades);

        if (parent.defs != null) {
            Color p = parent.defs.texInfo.getTints(parent.globalpal);
        ...
        boolean rendered = m.render(parent.globalpal, parent.globalshade, 0, (int) (parent.globalfog.combvis),
                polyColor.a);
...
//		x0 += xoff * (tspr.xrepeat >> 2);
//		k0 -= ((yoff * tspr.yrepeat) << 2);
        posx += tspr.getXrepeat() >> 2;
        posz -= (tspr.getYrepeat() << 2);

        // yoffset differs from zadd in that it does not follow cstat&8 y-flipping
        modela0.z += m.getYOffset(false) * mscale;

        f = (tspr.getXrepeat()) / 64.0f * m.getScale();
        float z = (tspr.getYrepeat()) / 64.0f * m.getScale();
        cScale.scl(-f, f, z);
        nScale.scl(-f, f, z);
        modela0.scl(f, -f, z);
        ...
        if ((globalorientation & 48) == 32) {
            ...
            modela0.x = modela0.z;
            modela0.z = f;
            posy += (pic.getHeight() * tspr.getYrepeat()) >> 3;
        }

        f = (65536.0f * 512.0f) / (parent.xdimen * parent.viewingrange);
        g = (float) (32.0f / (parent.xdimen * parent.gxyaspect));
        cScale.scl(f, -f, g);
        nScale.scl(f, -f, g);

        modela0.y = ((posx - parent.globalposx) / 1024.0f + modela0.y) * f;
        modela0.x = ((posy - parent.globalposy) / 1024.0f + modela0.x) * f;
        modela0.z = ((posz - parent.globalposz) / -16384.0f + modela0.z) * g;
...
//    	md3_vox_calcmat_common(tspr, dvoxa0);
        ...
        int numshades = engine.getPaletteManager().getShadeCount();
        polyColor.r = polyColor.g = polyColor.b = (numshades
                - min(max((parent.globalshade * parent.shadescale) + m.getShadeOff(), 0), numshades)) / (numshades);

        DefScript defs = parent.defs;
        if (defs != null) {
            if (m.isTintAffected() || (boardService.isValidSprite(tspr.getOwner()) && boardService.getSector(boardService.getSprite(tspr.getOwner()).getSectnum()).getFloorpal() != 0)) {
                Color p = defs.texInfo.getTints(parent.globalpal);
                polyColor.r *= p.r / 255.0f;
                polyColor.g *= p.g / 255.0f;
                polyColor.b *= p.b / 255.0f;
    ...
    public boolean mddraw(MDModel m, Sprite tspr) {
        ...
        m.updateanimation(defs.mdInfo, tspr, engine.getTimer().getTicsPerSecond(), parent.getConfig().isAnimSmoothing());
        ...
        return m.render(parent.globalpal, parent.globalshade, t2m.skinnum, parent.globalvisibility, 1.0f);
    }
    ...
    public void md3_vox_calcmat_common(Sprite tspr, Vector3 a0, float f, float[][] mat) {
        ...
        k0 = (tspr.getX() - parent.globalposx) * f / 1024.0f;
        k1 = (tspr.getY() - parent.globalposy) * f / 1024.0f;

        f = gcosang2 * gshang;
        g = gsinang2 * gshang;
        Spriteext sprext = parent.defs.mapInfo.getSpriteInfo(tspr.getOwner());

        k4 = EngineUtils.cos(tspr.getAng() + (sprext != null ? sprext.angoff : 0) + 512) / 16384.0f;
        k5 = EngineUtils.sin(tspr.getAng() + (sprext != null ? sprext.angoff : 0) + 512) / 16384.0f;
        k2 = k0 * (1 - k4) + k1 * k5;
        k3 = k1 * (1 - k4) - k0 * k5;
    ...
    public void md3_vox_calcmat_common(Sprite tspr, Vector3 a0, float f, Matrix4 mat) {
        float yaw = parent.globalang / (2048.0f / 360.0f) - 90.0f;
        float roll = parent.gtang * 57.3f; // XXX 57.3f WFT
        Spriteext sprext = parent.defs.mapInfo.getSpriteInfo(tspr.getOwner());

        float spriteang = ((tspr.getAng() + (sprext != null ? sprext.angoff : 0) + 512) & 2047) / (2048.0f / 360.0f);
        ...
        mat.rotate(-1.0f, 0.0f, 0.0f, parent.pitch);
    ...
    public void md3_vox_calcmat_common(Sprite tspr, Vector3 a0) {
        ...
        gl.glRotatef(parent.pitch, -1, 0, 0);
