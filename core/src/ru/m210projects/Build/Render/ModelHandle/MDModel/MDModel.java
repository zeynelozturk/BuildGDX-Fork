...
//				Console.out.println("Using def skin 0,0 as fallback, pal = " + pal, OsdColor.YELLOW);
    ...
    public void updateanimation(ModelsInfo mdInfo, Sprite tspr, int timerticspersec, boolean animSmoothing) {
        ...
        cframe = nframe = t2m.framenum;
        boolean smoothdurationp = (animSmoothing && (t2m.smoothduration != 0));

        Spritesmooth smooth = mdInfo.getSmoothParams(tspr.getOwner());
        SpriteAnim sprext = mdInfo.getAnimParams(tspr.getOwner());
        ...
        int fps = (smooth.mdsmooth != 0) ? Math.round((1.0f / (t2m.smoothduration)) * 66.f)
                : anim.fpssc;
        ...
        // use of MOD
        {
            sprext.mdanimtims += j / (((long) fps * timerticspersec) / 120);
        }
